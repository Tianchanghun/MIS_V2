...
def final_legacy_migration():
    """올바른 스키마로 656개 제품과 834개 상세를 완전 마이그레이션"""
    print("🚀 최종 레거시 DB 마이그레이션 (올바른 스키마)")
    print("=" * 70)
    ...
    try:
        ...
        # 5. 제품 마스터 배치 삽입 (올바른 스키마)
        print("\n5️⃣ 제품 마스터 배치 삽입 (올바른 스키마)")
                    ...
                    try:
                        ...
                        result = db.session.execute(db.text("""
                            INSERT INTO products (
                                ...
                                category_code_seq, type_code_seq, is_active, 
                                legacy_seq, created_at, updated_at
                            ) VALUES (
                                :company_id, :product_name, :price, :brand_seq,
                                :category_seq, :type_seq, :is_active,
                                :legacy_seq, :created_at, :updated_at
                            ) RETURNING id
                        """), {
                            ...
                            'type_seq': type_seq,
                            'is_active': True,  # status 대신 is_active 사용
                            'legacy_seq': product[0],  # 레거시 Seq 저장
            ...
            for i in range(0, len(legacy_details), detail_batch_size):
                ...
                for detail in batch:
                    try:
                        ...
                        # 상세 삽입 (product_details는 status 컬럼 있음)
                        ...
                        """), {
                            ...
                            'legacy_seq': detail[0],  # 레거시 Seq 저장
        ...
        with app.app_context():
            ...
            # 샘플 데이터 확인
            result = db.session.execute(db.text("""
                SELECT p.product_name, p.price, COUNT(pd.id) as detail_count
                FROM products p
                LEFT JOIN product_details pd ON p.id = pd.product_id
            ...
            for sample in samples:
                print(f"      {sample.product_name}: {sample.price:,}원, {sample.detail_count}개 상세")
