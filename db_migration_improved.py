...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('migration_improved.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class ImprovedDBMigrator:
    def __init__(self):
    ...
    def verify_migration_results(self, mssql_conn, postgres_conn) -> Dict[str, Dict]:
        """마이그레이션 결과 검증"""
    ...
    def run_migration(self) -> bool:
        ...
        try:
            ...
            for table_name in self.table_priority:
                ...
                # 중간 진행상황 출력
                if count > 0:
                    logger.info(f"✅ {table_name} 완료: {count:,}건")
                else:
                    logger.warning(f"⚠️ {table_name} 처리 없음: {count}건")
            ...
            logger.info(f"📦 총 마이그레이션 레코드: {total_records:,}건")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("="*60)
        print("📝 상세 로그: migration_improved.log")
        ...
        print("="*60)
        print("📝 오류 로그: migration_improved.log")
        print("="*60) 